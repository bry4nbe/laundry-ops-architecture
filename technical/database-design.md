...
Los estados posibles son: `pending_send → sent → returned`, representados con `TextChoices`. El valor `NULL` indica que el item no es lavado al seco. La granularidad por item es la que permite determinar si aún quedan prendas con el tercero al momento de consultar el estado derivado de la orden.

---

### 4. `payment_type` como campo operativo

> **Pendiente de confirmar contra implementación real** en `laundry-ops-api`. Los valores `advance`, `partial` y `final` se documentan según el diseño original; verificar que el modelo actual los mantenga tal cual.
...
`order_number` se genera en `services.py` dentro de `transaction.atomic()` usando el `id` autoincremental asignado por PostgreSQL. Los gaps en la numeración son aceptables (transacciones abortadas pueden dejar huecos).

---

### 6. `reference_code` en `payments`

`payments` incluye `reference_code` como campo opcional para el número de operación de Yape o Plin.
...
- `order_items.notes`: Información operativa para protección del negocio en la zona de lavado (ej. "Camisa ya vino con mancha de vino").
...
`expenses` registra los gastos operativos del negocio, desacoplados de las órdenes para no contaminar el modelo financiero principal.
...
| `dry_cleaning_status` en `order_items` (TextChoices) | Campo en `orders` | Estado por prenda, no por orden |
...
| Modelo `Expense` separado | Campo en `orders` | Gastos operativos desacoplados de las órdenes |
